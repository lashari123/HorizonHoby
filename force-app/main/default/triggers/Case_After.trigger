trigger Case_After on Case (after insert, after update, after delete, after undelete) 
{
    ...
    if(trigger.isInsert)
    {
        if(!SystemUtilities.isAlreadyExecuted('CaseTriggers.updateCaseFields'))
        {
            CaseTriggers.updateCaseFields(trigger.new, null, TriggerType.isInsert);            
        }
    }
    ...
    if(trigger.isUpdate)
    {
        if(!SystemUtilities.isAlreadyExecuted('CaseTriggers.updateCaseFields'))
        {
            CaseTriggers.updateCaseFields(trigger.new, trigger.oldMap, TriggerType.isUpdate);
        }
    }
}